...
import { ChevronRight, Package, Activity, Printer, FileCheck, Ship, Factory, Truck, CheckCircle, XCircle } from 'lucide-react';
...
function Node({ icon, color, title, subtitle, isLast = false, isPending = false }) {
  const colorRing = {
    blue: 'bg-blue-600',
    ...
    <div className="flex gap-3">
      <div className="flex flex-col items-center flex-shrink-0">
        <div className={`w-8 h-8 ${ring} rounded-full flex items-center justify-center`}>
          {createElement(icon, { className: `w-4 h-4 ${isPending ? 'text-gray-400' : 'text-white'}` })}
        </div>
...
const BoxDetail = ({ box, inboundMaterial, lcs, onBack }) => {
  ...
  const warehouseData = shipment?.stepData?.warehouse || null;
  const warehouseStatus = warehouseData?.warehouse_status || null;
  const dispatchDate = warehouseData?.dispatched_at || warehouseData?.updated_at || shipment?.updated_at || null;
  ...
  const statusCfg = {
    ...
    Consumed: { icon: XCircle, bg: 'bg-gray-100', tx: 'text-gray-700' },
  };
  ...
  const issueDate = fmtDate(box.issue_date);
  const issueShift = box.issue_shift ? `${box.issue_shift} Shift` : null;
  const receiveDate = fmtDateTime(box.created_at);
  const dispatchSubtitle = warehouseStatus === 'dispatched' || warehouseStatus === 'received'
    ? `Warehouse dispatched on ${fmtDate(dispatchDate) || 'recorded date unavailable'}`
    : null;
  const issueSubtitle = isInStock
    ? 'Waiting to be issued to production floor'
    : `${issueDate || 'Issue date unavailable'}${issueShift ? ` - ${issueShift}` : ''}`;

  let consumeSubtitle = 'Not yet started';
  if (isConsumed) {
    consumeSubtitle = `All ${box.quantity.toLocaleString()} units used in production on ${fmtDate(box.updated_at || box.created_at) || 'recorded date unavailable'}`;
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center gap-1 px-3 py-1.5 text-sm font-semibold rounded-full ${cfg.bg} ${cfg.tx}`}>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div>
          ...
          <Node
            icon={Truck}
            color={dispatchSubtitle ? 'teal' : 'gray'}
            title="Box Dispatched"
            subtitle={dispatchSubtitle || 'Awaiting warehouse dispatch update'}
            isPending={!dispatchSubtitle}
          />
          <Node
            icon={Package}
            color="teal"
            title="Box Receive Date"
            subtitle={receiveDate ? `${box.box_name} received on ${receiveDate}` : 'Receive date unavailable'}
            isPending={!receiveDate}
          />
          <Node
            icon={Factory}
          ...
          <Node
            icon={Activity}
            color={isConsumed ? 'green' : isInProd ? 'orange' : 'gray'}
            title="Consumption"
