...
const ShipmentDetail = ({ lc, shipment, onBack, onUpdateShipment, onComplete }) => {
  ...
  useEffect(() => {
    if (shipment) {
      ...
      setFormData(prev => ({
        ...
        date: '', document_no: '', lc_value_foreign_realised: '', exchange_rate: '', lc_value_bdt_realised: '', interest_amount: '',
  ...
  const handleInputChange = (field, value) => {
    setFormData(prev => {
      ...
      // Auto-compute LC Value BDT Realised from Foreign * Exchange Rate
      if (field === 'lc_value_foreign_realised' || field === 'exchange_rate') {
        const foreign = parseFloat(field === 'lc_value_foreign_realised' ? value : next.lc_value_foreign_realised) || 0;
        const rate    = parseFloat(field === 'exchange_rate'              ? value : next.exchange_rate)              || 0;
        next.lc_value_bdt_realised = foreign > 0 && rate > 0 ? (foreign * rate).toFixed(2) : '';
      }
      return next;
    });
  };
    ...
    setFormData(prev => {
      ...
      return {
        ...
        customsClearedGoods: [...prev.customsClearedGoods, {
          ...
          quantity_per_box: '', missing_quantity: '0', missing_boxes: '0',
  ...
  // applyWarehouseItemUpdate — the actual mutation after any rebox confirmation
  const applyWarehouseItemUpdate = (index, field, value) => {
    const updated = [...formData.customsClearedGoods];
    updated[index] = { ...updated[index], [field]: value, updated_at: new Date().toISOString() };

    // Effective box count = no_of_boxes - missing_boxes (missing_boxes can't exceed no_of_boxes)
    const getEffectiveBoxes = (item) => {
      const nob = parseInt(item.no_of_boxes) || 0;
      const mb  = parseInt(item.missing_boxes) || 0;
      return Math.max(0, nob - mb);
    };

    if (field === 'no_of_boxes' || field === 'quantity_per_box' || field === 'missing_boxes') {
      ...
      const mb  = parseInt(field === 'missing_boxes'     ? value : updated[index].missing_boxes)     || 0;
      const clamped = Math.min(mb, nob); // can't have more missing than total
      updated[index].missing_boxes = clamped;
      const effectiveBoxes = Math.max(0, nob - clamped);
      // Total qty = effective boxes * qty/box (read-only, auto-computed)
      updated[index].quantity = effectiveBoxes * qpb;
      // Regenerate boxes using effective count
      updated[index].boxes = generateBoxesForItem({ ...updated[index], no_of_boxes: effectiveBoxes });
    }
  ...
  const updateWarehouseItem = (index, field, value) => {
    const item = formData.customsClearedGoods[index];
    // If this item already has saved boxes (from a previous draft) and user is changing
    // a structural field, show a warning before wiping the existing boxes
    const isStructuralField = field === 'no_of_boxes' || field === 'quantity_per_box';
    const hasSavedBoxes = item.boxes && item.boxes.length > 0 &&
      item.boxes.some(b => (b.missing_qty || 0) > 0 || (b.extra_qty || 0) > 0 || (b.remarks || '') !== '');
    if (isStructuralField && hasSavedBoxes) {
      setReboxConfirm({ index, field, value });
      return;
    }
    applyWarehouseItemUpdate(index, field, value);
  };
  ...
  const handleSave = () => {
    ...
    if (currentStep === 1) {
      ...
      newData.bank_interest = {
        ...
        lc_value_foreign_realised: parseFloat(formData.lc_value_foreign_realised) || 0,
      ...
      newData.warehouse = {
        ...
        total_missing_boxes: itemsWithBoxes.reduce((s, i) => s + (parseInt(i.missing_boxes) || 0), 0),
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input type="text" value={formData.document_no || ''} placeholder="Enter document number"
                  onChange={e => handleInputChange('document_no', e.target.value)}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
              </div>
              {/* LC Value Foreign Realised */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">LC Value Foreign Realised *</label>
                <input type="number" value={formData.lc_value_foreign_realised || ''} placeholder="0.00"
                  onChange={e => handleInputChange('lc_value_foreign_realised', e.target.value)}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
              </div>
              {/* Exchange Rate */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Exchange Rate *</label>
                <input type="number" value={formData.exchange_rate || ''} placeholder="e.g. 110.00"
                  onChange={e => handleInputChange('exchange_rate', e.target.value)}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
              </div>
              {/* LC Value BDT Realised — auto-computed, read-only */}
              <div>
                ...
                <div className="w-full px-4 py-2 border border-indigo-200 bg-indigo-50 rounded-lg text-sm font-semibold text-indigo-900 min-h-[42px]">
                  {formData.lc_value_bdt_realised
                    ? `৳ ${parseFloat(formData.lc_value_bdt_realised).toLocaleString()}`
                    : <span className="text-gray-400 font-normal">Computed from Foreign × Rate</span>}
                </div>
              </div>
              {/* Interest Amount */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Interest Amount (BDT) *</label>
                <input type="number" value={formData.interest_amount || ''} placeholder="0.00"
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Missing Boxes</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {formData.customsClearedGoods.map((item, idx) => {
                      ...
                      const totalQty = parseInt(item.quantity) || 0;
                      const noBoxes  = parseInt(item.no_of_boxes) || 0;
                      const missingBoxes = parseInt(item.missing_boxes) || 0;
                      const effectiveBoxes = Math.max(0, noBoxes - missingBoxes);
                      ...
                      return (
                        <>
                          ...
                          <tr key={`item-${idx}`}
                            ...
                            <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                              {isWarehouseLocked ? (
                                ...
                                <input type="number" className="w-20 px-2 py-1 border rounded text-sm focus:ring-2 focus:ring-green-500"
                            ...
                            {/* Total Qty — read-only, auto-computed from (no_of_boxes - missing_boxes) * qty_per_box */}
                            <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                              <div className="w-28 px-2 py-1 bg-gray-50 border border-gray-200 rounded text-sm font-medium text-gray-700 text-center">
                                {(parseInt(item.quantity) || 0).toLocaleString()}
                              </div>
                            </td>
                            {/* Missing Boxes — plain style, same as other input cells */}
                            <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                              {isWarehouseLocked ? (
                                ...
                                <input type="number" min="0"
                                  className="w-20 px-2 py-1 border rounded text-sm focus:ring-2 focus:ring-green-500"
                                  value={item.missing_boxes || ''} placeholder="0"
                                  onChange={e => updateWarehouseItem(idx, 'missing_boxes', e.target.value)} />
                              )}
                            </td>
                              ...
                              {noBoxes > 0 ? (
                                <div className="flex flex-col gap-1">
                                  <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-700">
                                    <Package className="w-3 h-3" /> {effectiveBoxes} boxes
                                  </span>
                                  {missingBoxes > 0 && (
                                    <span className="text-xs text-red-500">{missingBoxes} missing</span>
                                  )}
                                </div>
                              ) : (
                          ...
                          {isExpanded && boxes.length > 0 && (
                            <tr key={`boxes-${idx}`}>
                              <td colSpan="9" className="px-0 py-0 border-b border-gray-200 bg-gray-50">
              ...
              <div className={`border rounded-lg p-4 flex items-center justify-between ${
                ...
                <div className="flex gap-6 text-sm">
                  ...
                  <span className="text-gray-600">Total Boxes: <strong className="text-green-700">
                    {formData.customsClearedGoods.reduce((s, i) => s + Math.max(0, (parseInt(i.no_of_boxes)||0) - (parseInt(i.missing_boxes)||0)), 0)}
                  </strong></span>
                  {formData.customsClearedGoods.some(i => parseInt(i.missing_boxes) > 0) && (
                    <span className="text-gray-600">Missing Boxes: <strong className="text-red-600">
                      {formData.customsClearedGoods.reduce((s, i) => s + (parseInt(i.missing_boxes) || 0), 0)}
                    </strong></span>
                  )}
                </div>
                {/* Inline status badge */}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h3 className="text-base font-bold text-gray-900 mb-1">Regenerate Boxes?</h3>
                <p className="text-sm text-gray-600">
                  This item already has box data with missing/extra entries or remarks recorded.
                  Changing the box count or quantity per box will <strong>clear all existing box data</strong> for this item.
                </p>
              </div>
            </div>
            <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-5">
              ⚠ All missing, extra, and remark data for this item's boxes will be permanently removed.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setReboxConfirm(null)}
                className="flex-1 py-2 border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors">
                Cancel
              </button>
              <button
                onClick={() => {
                  // Clear existing box data then apply the change
                  const { index, field, value } = reboxConfirm;
                  const updated = [...formData.customsClearedGoods];
                  updated[index] = { ...updated[index], boxes: [] };
                  setFormData(prev => ({ ...prev, customsClearedGoods: updated }));
                  setReboxConfirm(null);
                  // Apply after state settle
                  setTimeout(() => applyWarehouseItemUpdate(index, field, value), 0);
                }}
                className="flex-1 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg text-sm font-medium transition-colors">
                Yes, Regenerate
              </button>
            </div>
          </div>
