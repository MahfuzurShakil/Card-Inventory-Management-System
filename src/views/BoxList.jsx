...
const getBoxType = (box) => {
  const itemType = normalizeText(box.item_type);
  const itemName = normalizeText(box.item_name);

  if (itemType.includes('chip') || itemName.includes('chip')) return 'Chip';
  if (itemType.includes('tape') || itemName.includes('tape')) return 'Tape';
  if (itemType.includes('sheet') || itemName.includes('sheet')) return 'Sheet';
  return null;
};

const getBreakdown = (items) => {
  const counts = { Chip: 0, Tape: 0, Sheet: 0 };
  items.forEach((box) => {
    const type = getBoxType(box);
    if (type) counts[type] += 1;
  });

  return ['Chip', 'Tape', 'Sheet'].map((type) => ({
    label: type,
    value: counts[type],
  }));
};

const BreakdownRow = ({ items }) => {
  const breakdown = getBreakdown(items);
  ...
  return (
    <p className="mt-1.5 text-xs text-gray-500">{text || 'No box breakdown available'}</p>
  );
};

const getItemBadgeClass = (box) => {
  const type = getBoxType(box);
  if (type === 'Chip') return 'bg-blue-100 text-blue-800';
  if (type === 'Tape') return 'bg-purple-100 text-purple-800';
  if (type === 'Sheet') return 'bg-green-100 text-green-800';
...
const BoxList = ({ boxes, inboundMaterials, lcs, onViewBox, onIssueToProduction }) => {
  ...
  const filteredBoxes = boxes.filter((box) => {
    const shipmentNumber = getShipmentNumber(box);
    const quickSearch = normalizeText(appliedFilters.quickSearch);
    const boxNameSearch = normalizeText(appliedFilters.boxName);
    const type = getBoxType(box);

    const matchesQuickSearch = !quickSearch
      || normalizeText(box.box_name).includes(quickSearch)
      || normalizeText(box.barcode).includes(quickSearch)
      || normalizeText(box.item_name).includes(quickSearch)
      || normalizeText(shipmentNumber).includes(quickSearch);

    const matchesStatus = appliedFilters.status === 'all' || box.status === appliedFilters.status;
    const matchesBoxName = !boxNameSearch || normalizeText(box.box_name).includes(boxNameSearch);
    const matchesItemType = appliedFilters.itemType === 'all' || type === appliedFilters.itemType;

    return matchesQuickSearch && matchesStatus && matchesBoxName && matchesItemType;
  });
  ...
  const toggleAll = () => {
    if (allPgSel) setSelectedBoxIds((prev) => prev.filter((id) => !pageIds.includes(id)));
    else setSelectedBoxIds((prev) => [...new Set([...prev, ...pageIds])]);
  };
  ...
  const inStockBoxes = boxes.filter((b) => b.status === 'Material In Stock');
  const inProdBoxes = boxes.filter((b) => b.status === 'Material In Production');
  const consumed = boxes.filter((b) => b.status === 'Consumed').length;
  const remainingBoxes = boxes.filter((b) => b.status !== 'Consumed');
  const statusCfg = {
  ...
  const statCards = [
    ...
    {
      label: 'In Stock',
      value: inStockBoxes.length,
      items: inStockBoxes,
      icon: CheckCircle,
      iconBg: 'bg-green-100',
      iconCl: 'text-green-600',
      valCl: 'text-green-900',
    },
    {
      label: 'In Production',
      value: inProdBoxes.length,
      items: inProdBoxes,
      icon: Activity,
      iconBg: 'bg-blue-100',
    ...
    },
    {
      ...
      items: remainingBoxes,
      icon: Barcode,
      iconBg: 'bg-orange-100',
      iconCl: 'text-orange-600',
      valCl: 'text-gray-900',
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {statCards.map(({ label, value, items, icon, iconBg, iconCl, valCl }) => (
          <div key={label} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <BreakdownRow items={items} />
              </div>
              <div className={`w-10 h-10 ${iconBg} rounded-lg flex items-center justify-center flex-shrink-0`}>
                {createElement(icon, { className: `w-5 h-5 ${iconCl}` })}
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="p-3.5 md:p-4 border-b border-gray-100">
          <div className="flex flex-col xl:flex-row gap-2.5">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search by shipment number, box name, barcode or item..."
        ...
        {showMoreFilters && (
          <div className="p-3.5 md:p-4 bg-gray-50/70">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">Box Name</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    value={draftFilters.boxName}
                    onChange={(e) => updateDraftFilter('boxName', e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') applyFilters();
                    }}
                  />
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => updateDraftFilter('itemType', e.target.value)}
                >
              ...
              {paginatedBoxes.length > 0 ? paginatedBoxes.map((box) => {
                ...
                return (
                  <tr key={box.id} className={`hover:bg-gray-50 transition-colors ${selected ? 'bg-blue-50' : ''}`}>
                    ...
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{sn}</td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 text-xs font-semibold rounded ${getItemBadgeClass(box)}`}>
                        {box.item_type || box.item_name || 'N/A'}
                      </span>
                    </td>
                ...
                <tr>
                  <td colSpan="10" className="px-6 py-12 text-center text-sm text-gray-500">
                    {appliedFilters.quickSearch || appliedFilters.status !== 'all' || appliedFilters.boxName || appliedFilters.itemType !== 'all' ? 'No boxes found matching your filters.' : (
                      <div>
